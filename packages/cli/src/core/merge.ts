...
export function findBlock(lines: string[]): FindResult {
  ...
  if (
    ...
    return {
      kind: 'error',
      message: `expected exactly one ${BEGIN} … ${END} span, found ${begins.length} BEGIN and ${ends.length} END`,
    }
  }
  if (end < begin)
    return { kind: 'error', message: `${END} comes before ${BEGIN}` }
...
export async function ensureClaudeMd(
  dir: string
): Promise<'created' | 'prepended' | 'unchanged'> {
